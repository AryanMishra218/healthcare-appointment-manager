...
| **Email notifications** | Booking confirmation, cancellation, reminders via SendGrid |
...
| `SENDGRID_API_KEY` | Optional | From [sendgrid.com](https://sendgrid.com). Without it, emails are logged as failed but booking works. |
| `SENDGRID_FROM_EMAIL` | Optional | The verified sender email in your SendGrid account. |
...
> **Note:** All external services (Gemini, SendGrid, Google Calendar) are optional for local development.
...
Tests use mocking for all external APIs (Gemini, SendGrid, Google Calendar)
...
│   │   ├── email_service.py # SendGrid integration, retry logic
